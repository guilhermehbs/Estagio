...
public class Registra {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		for (Escritor escritor : escritores) {
			escritor.escreverSaida(list); // For que passa pela lista e escreve os números no txt e xlsx
		}
	...
	// Método para a entrada dos números, validação deles e adcionar na lista
	private static void entradaDeDadosEValidacao(Scanner scan, List<Integer> list, int quantidadeDeNumeros) {
		...
		for (int i = 1; i <= quantidadeDeNumeros; i++) {
			...
			if (valida) { // Se o número não for inteiro nem entre 0 e 10, digitar outro
